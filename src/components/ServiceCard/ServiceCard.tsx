...
export const ServiceCard = memo(function ServiceCard({ service }: ServiceCardProps) {
  return (
    <motion.article
      ...
      whileHover={{ y: -4 }}
      className="group relative flex h-[430px] min-w-[292px] shrink-0 basis-[86%] flex-col overflow-visible rounded-[28px] md:basis-[calc((100%_-_48px)/3)]"
    >
      <div className="relative flex h-full flex-col overflow-hidden rounded-[28px] border border-white/42 bg-white/8 shadow-[0_22px_60px_rgba(0,0,0,0.16)] backdrop-blur transition duration-300 group-hover:border-white/58 group-hover:bg-white/12 group-hover:shadow-[0_28px_70px_rgba(0,0,0,0.22)]">
        ...
        <div className="flex h-[116px] shrink-0 items-center border-b border-white/28 px-8 py-5">
          <h3 className="max-w-[12ch] text-[1.55rem] font-semibold leading-[1.12] text-white md:text-[1.65rem]">{service.title}</h3>
        </div>

        <div className="relative mt-auto flex h-[314px] shrink-0 items-end px-0 pb-0 pt-16">
          ...
          <button
            ...
            className="absolute bottom-0 right-0 z-20 grid h-[88px] w-[88px] place-items-center rounded-full border border-white/10 bg-dark text-white shadow-[0_18px_40px_rgba(0,0,0,0.26)] transition duration-300 group-hover:scale-[1.04] group-hover:border-secondary/50 group-hover:bg-secondary group-hover:text-dark md:h-24 md:w-24"
          >
            <ArrowUpRight size={46} strokeWidth={1.8} />
          </button>
        </div>
      </div>
    </motion.article>
